...
// geolocator is used from UI; repository stores coordinates only.

import '../../core/location/place_label.dart';
import '../../models/entry.dart';
import '../auth/auth_providers.dart';
import 'local_entries_store.dart';
import 'music_attachment_utils.dart';
...
class EntriesRepository {
  ...
  Stream<List<Entry>> watchEntriesForMonth(DateTime month) {
    ...
    final startId = dateId(firstDay);
    final endId = dateId(lastDay);
  ...
  Stream<List<Entry>> watchEntriesForRange({
    ...
    final startId = dateId(from);
    final endId = dateId(to);
    final user = _authState.value;
  ...
  Future<void> upsertEntryForDateId({
    ...
    final day = tryParseDateId(dateId);
    if (day == null) {
  ...
  void _watchRemoteEntry({required String uid, required String dateId}) {
    ...
    _entrySubscriptions[key] = _entryRef(uid: uid, dateId: dateId)
        ...
        .listen((doc) {
          ...
          unawaited(
            _localStore.upsertRemote(uid: uid, remote: Entry.fromDoc(doc)),
          );
        });
  }
